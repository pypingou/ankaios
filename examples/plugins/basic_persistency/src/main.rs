...
async fn persist_workload(
    workload_name: &str,
    signed_yaml: &str,  // Complete signed YAML for this workload
    workloads_dir: &Path,
) -> Result<(), Box<dyn std::error::Error>> {
    // Ensure workloads directory exists
    tokio::fs::create_dir_all(workloads_dir).await?;

    // SECURITY REQUIREMENT: Validate signed_yaml has signature block
    // Runtime state is on writable filesystem and must be signed to prevent tampering
    if !signed_yaml.contains("\n---\n") || !signed_yaml.contains("signature:") {
        return Err(format!(
            "Workload '{}' missing signature block. \
            Configure server with signature_verification.require_signature=true",
            workload_name
        ).into());
    }
    ...
    let temp_file = safe_join(&workloads_dir, &format!(".{}.yaml.tmp", workload_name))
        .map_err(|e| format!("Invalid workload name '{}': {}", workload_name, e))?;

    // Atomic write: temp file then rename
    tokio::fs::write(&temp_file, signed_yaml).await?;
    ...
    log::info!("Persisted workload '{}' to {:?}", workload_name, workload_file);
    Ok(())
}
...
async fn remove_persisted_workload(
    ...
    if workload_file.exists() {
        tokio::fs::remove_file(&workload_file).await?;
        log::info!("Removed persisted workload '{}' from {:?}", workload_name, workload_file);
    }
...
async fn load_persisted_state(
    ...
    while let Some(entry) = entries.next_entry().await? {
        ...
        // Skip non-.yaml files and temp files
        let has_yaml_ext = path.extension()
            .and_then(|ext| ext.to_str())
            .map_or(false, |ext| ext == "yaml");

        if !has_yaml_ext {
            log::debug!("Skipping (not yaml): {:?}", path);
            continue;
        }
        ...
        match tokio::fs::read_to_string(&path).await {
            Ok(content) => {
                ...
                // Parse signed YAML - extract only the first document (before signature block)
                let yaml_to_parse = if let Some(separator_pos) = content.find("\n---\n") {
                    &content[..separator_pos]
                } else {
                    &content
                };
                match serde_yaml::from_str::<State>(yaml_to_parse) {
                    Ok(workload_state) => {
                        ...
                        if let Some(ref workloads) = workload_state.workloads {
                            for (name, workload) in &workloads.workloads {
                                ...
                                log::info!("Loaded persisted workload '{}' from {:?}", name, path);
                            }
                        }
                    }
                    Err(e) => {
...
async fn process_event(
    ...
    on_running_signed_yamls: &mut HashMap<String, String>,
) -> Result<(), Box<dyn std::error::Error>> {
    // Log event details
            ...
            if let Some(mode) = get_persist_mode(&workload.tags) {
                match mode.as_str() {
                    ...
                    "ON_RUNNING" => {
                        ...
                        // Save the signed_yaml for later when the workload reaches Running state
                        ...
                        if !response.signed_yaml.is_empty() {
                            on_running_signed_yamls.insert(workload_name.to_string(), response.signed_yaml.clone());
                            log::debug!("Saved signed_yaml for ON_RUNNING workload '{}'", workload_name);
                        } else {
                            log::warn!("ON_RUNNING workload '{}' has no signed_yaml in event", workload_name);
                        }
                    }
                    _ => {}
                }
                                ...
                                if let Some(mode) = get_persist_mode(&workload.tags) {
                                    match mode.as_str() {
                                        "ALWAYS" => {
                                            ...
                                            log::info!("Workload '{}' updated, re-persisting with new signature", workload_name);

                                            if !response.signed_yaml.is_empty() {
                                                let workloads_dir = get_workloads_dir(persistence_file);
                                                if let Err(e) = persist_workload(workload_name, &response.signed_yaml, &workloads_dir).await {
                                                ...
                                                log::warn!("Workload '{}' updated but no signed_yaml in event", workload_name);
                                            }
                                        }
                                        "ON_RUNNING" => {
                                            // Update the cached signed_yaml for ON_RUNNING workloads
                                            log::debug!("Workload '{}' (ON_RUNNING) updated, updating cached signed_yaml", workload_name);
                                            if !response.signed_yaml.is_empty() {
                                                on_running_signed_yamls.insert(workload_name.to_string(), response.signed_yaml.clone());
                                            }
                                        }
                        ...
                        if let Some(complete_state) = &response.complete_state {
                            ...
                            if is_running {
                                ...
                                if !already_persisted {
                                    ...
                                    // Use the saved signed_yaml for this workload
                                    if let Some(signed_yaml) = on_running_signed_yamls.get(workload_name) {
                                        let workloads_dir = get_workloads_dir(persistence_file);
                                        if let Err(e) = persist_workload(workload_name, signed_yaml, &workloads_dir).await {
                                        ...
                                        log::error!("ON_RUNNING workload '{}' reached Running but signed_yaml was not saved", workload_name);
                                    }
                                }
                            }
                        }
    ...
    if modified {
        // Use signed_yaml from the event (contains the workload that was just added/updated)
        let signed_yaml_to_use = &response.signed_yaml;

        if !signed_yaml_to_use.is_empty() {
            // Parse to get workload names and persist each one to its own file
            let workloads_dir = get_workloads_dir(persistence_file);

            // Extract first YAML document (before signature block separator)
            let yaml_to_parse = if let Some(separator_pos) = signed_yaml_to_use.find("\n---\n") {
                &signed_yaml_to_use[..separator_pos]
            } else {
                signed_yaml_to_use
            };
            ...
            match serde_yaml::from_str::<State>(yaml_to_parse) {
                Ok(state_from_yaml) => {
                    if let Some(ref workloads) = state_from_yaml.workloads {
                        for workload_name in workloads.workloads.keys() {
                            persist_workload(workload_name, signed_yaml_to_use, &workloads_dir).await?;
                        }
                    }
                }
                Err(e) => {
                    log::warn!("Failed to parse signed_yaml for persistence: {}", e);
                }
            }
        } else {
            log::warn!("ALWAYS workload modified but no signed_yaml in event");
        }
    } else {
...
fn extract_signature_counter(signed_yaml: &str) -> Option<u64> {
    // Look for signature block after "---" separator
    let signature_block = signed_yaml.split("\n---\n").nth(1)?;

    // Parse signature block as YAML
    let signature_yaml: serde_yaml::Value = serde_yaml::from_str(signature_block).ok()?;

    // Extract counter field
    signature_yaml
        .get("counter")
        .and_then(|v| v.as_u64())
}
...
async fn restore_persisted_state(
    ...
    if let Some(ref workloads) = persisted_state.workloads {
        ...
        let mut workloads_with_counters: Vec<(String, Option<u64>)> = vec![];

        for workload_name in workloads.workloads.keys() {
            let workload_file = match safe_join(&workloads_dir, &format!("{}.yaml", workload_name)) {
                Ok(path) => path,
                Err(e) => {
                    log::error!("Invalid workload name '{}': {}", workload_name, e);
                    continue;
            ...
            // Extract counter from signature block
            let counter = if let Ok(signed_yaml) = tokio::fs::read_to_string(&workload_file).await {
                extract_signature_counter(&signed_yaml)
            } else {
                None
            };

            workloads_with_counters.push((workload_name.clone(), counter));
        }

        // Sort by counter (workloads without counters go first)
        workloads_with_counters.sort_by_key(|(_, counter)| *counter);

        log::debug!("Restoration order (by signature counter):");
        for (name, counter) in &workloads_with_counters {
            log::debug!("  - {}: counter={:?}", name, counter);
        }

        // For each persisted workload, read its file and send as UpdateStateRequest
        for (workload_name, _) in workloads_with_counters {
            let workload_file = match safe_join(&workloads_dir, &format!("{}.yaml", workload_name)) {
                Ok(path) => path,
                Err(e) => {
                    log::error!("Invalid workload name '{}': {}", workload_name, e);
            ...
            if let Ok(signed_yaml) = tokio::fs::read_to_string(&workload_file).await {
                // Extract first YAML document (before signature block separator)
                // Signed YAML has format: workload_yaml\n---\nsignature_block
                let yaml_to_parse = if let Some(separator_pos) = signed_yaml.find("\n---\n") {
                    &signed_yaml[..separator_pos]
                } else {
                    &signed_yaml
                };

                // Parse to get the workload state
                match serde_yaml::from_str::<State>(yaml_to_parse) {
                    Ok(workload_state) => {
                    // Generate update_mask for this workload
                    let update_mask = vec![format!("desiredState.workloads.{}", workload_name)];

                    // Create UpdateStateRequest with signed YAML
                        ...
                        to_ankaios_enum: Some(ToAnkaiosEnum::Request(Request {
                            ...
                            request_content: Some(RequestContent::UpdateStateRequest(Box::new(
                                UpdateStateRequest {
                                    ...
                                    update_mask,
                                    // Include signed YAML for signature verification
                                    signed_yaml: Some(signed_yaml),
                                },
                            ))),
                        })),
                    };
                    ...
                    Err(e) => {
                        log::error!("Failed to parse persisted workload '{}': {}", workload_name, e);
                        // Continue with other workloads even if one fails
                    }
                }
            }
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut on_running_signed_yamls: HashMap<String, String> = HashMap::new();
        ...
        match message.from_ankaios_enum {
            Some(FromAnkaiosEnum::Response(response)) => {
                ...
                if response.request_id == EVENT_REQUEST_ID {
                    ...
                    {
                        // Process the event
                        if let Err(e) = process_event(&state_response, &persistence_file, &mut output_pipe, &mut input_pipe, &mut on_running_workloads, &mut on_running_signed_yamls).await {
...
mod tests {
    ...
    async fn test_restore_persisted_state_success() {
        ...
        tokio::fs::write(workloads_dir.join("restored-workload.yaml"), &signed_yaml).await.unwrap();
...
"#;
    ...
    #[tokio::test]
    async fn test_persist_rejects_unsigned() {
        let temp_dir = std::env::temp_dir();
        ...
        // Create unsigned YAML (no signature block)
        let unsigned_yaml = serde_yaml::to_string(&state).unwrap();

        // Attempt to persist should FAIL (requires signature)
        let result = persist_workload("test-workload", &unsigned_yaml, &temp_workloads_dir).await;

        assert!(result.is_err(), "Should reject unsigned YAML");
        let err_msg = result.unwrap_err().to_string();
        assert!(err_msg.contains("missing signature block"), "Error should mention signature requirement");
        assert!(err_msg.contains("require_signature=true"), "Error should mention configuration requirement");
    ...
    async fn test_persist_multiple_workloads_separate_files() {
        ...
        let signed_yaml1 = create_mock_signed_yaml(&state1);

        persist_workload("mqtt_fedora", &signed_yaml1, &temp_dir).await.unwrap();

        // Create second workload
        let state2 = create_test_state_with_workload("mqtt_test", "ALWAYS");
        let signed_yaml2 = create_mock_signed_yaml(&state2);

        persist_workload("mqtt_test", &signed_yaml2, &temp_dir).await.unwrap();
    ...
    async fn test_load_persisted_state_from_directory() {
        ...
        persist_workload("workload1", &create_mock_signed_yaml(&state1), &temp_dir).await.unwrap();

        let state2 = create_test_state_with_workload("workload2", "ALWAYS");
        persist_workload("workload2", &create_mock_signed_yaml(&state2), &temp_dir).await.unwrap();
    ...
    async fn test_remove_persisted_workload() {
        ...
        persist_workload("test_workload", &create_mock_signed_yaml(&state), &temp_dir).await.unwrap();
    ...
    #[tokio::test]
    async fn test_signature_with_counter_preserved() {
        let temp_dir = std::env::temp_dir().join(format!("test_counter_{}", unique_test_id()));
        tokio::fs::create_dir_all(&temp_dir).await.unwrap();
        ...
        persist_workload("test", &signed_yaml, &temp_dir).await.unwrap();

        let persisted = tokio::fs::read_to_string(temp_dir.join("test.yaml")).await.unwrap();
        assert!(persisted.contains("counter: 1778665427"), "Counter field missing!");
        assert!(persisted.contains("signature: abc123"));
        assert!(persisted.contains("key_id: test-key"));
    ...
    async fn test_process_event_persists_always_workload() {
        ...
        // Verify file contains the signed YAML
        ...
        assert!(
            file_content.contains("signature:"),
            "File should contain signature block"
        );
        assert!(
            file_content.contains("counter:"),
            "File should contain counter field"
        );
    }
    ...
    async fn test_process_event_saves_on_running_signed_yaml() {
        ...
        // CRITICAL: Verify signed_yaml was saved for later persistence
        assert!(
            on_running_signed_yamls.contains_key("test_on_running"),
            "ON_RUNNING workload signed_yaml should be saved"
        );
        assert_eq!(
            on_running_signed_yamls.get("test_on_running").unwrap(),
            &signed_yaml,
            "Saved signed_yaml should match original"
        );
